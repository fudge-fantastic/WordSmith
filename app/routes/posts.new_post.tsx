...
export async function action({ request }: { request: Request }) {
    ...
    // console.log(authorId, title, slug, summary, description, category);
    ...
    try {
        await createPost(authorId, title, slug, summary, description, category);
            ...
            <div className="bg-vanila w-4/5 min-h-auto m-6 rounded-xl shadow-xl">
                <Form method="post" className="relative m-4 rounded-xl space-y-5" onSubmit={handleSubmit}>
                    <div className="flex md:flex-row justify-between gap-5 flex-col ">
                        ...
                        <Input name="authorId" type="hidden" value={userID as string} />
                    ...
                    <button
                        type="submit"
                        className="font-semibold border-2 border-vanila_text hover:bg-vanila_text rounded-full px-5 py-1 text-sm text-vanila_text hover:text-vanila duration-250"
